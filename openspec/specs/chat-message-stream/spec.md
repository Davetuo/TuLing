...
系统 SHALL 允许已登录用户发送文本消息，并通过Server-Sent Events流式接收AI生成的回复。
...
AI回复内容 SHALL 满足质量要求：语言清晰、格式结构化、旅行建议具备可执行步骤，涉及时效信息时附带标准免责声明。
...
系统 SHALL 将每条消息与会话ID、角色（user/assistant）、内容和创建时间戳一起持久化。
